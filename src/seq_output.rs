...
pub fn write_minimizers_and_seq_of_kmers(output_prefix: &PathBuf, node_indices: &mut HashMap<Kmer, usize>, kmer_origin: &HashMap<Kmer, String>, dbg_nodes: &HashMap<Kmer, u32> , k: usize, l: usize) {
    ...
    writeln!(out_file, "# k = {}", k).unwrap();
    writeln!(out_file, "# l = {}", l).unwrap();
    writeln!(out_file, "# Structure of remaining of the file:").unwrap();
    writeln!(out_file, "# [node ID]\t[list of minimizers]\t[sequence of node]\t[abundance]\t[origin]\t[shift]").expect("Error writing sequences file.");
        ...
        for line in lines {
            if let Ok(ip) = line {
                ...
                if index != &max_node {
                    ...
                    writeln!(out_file, "{}", s_line).expect("Error writing minimizers/sequences.");
